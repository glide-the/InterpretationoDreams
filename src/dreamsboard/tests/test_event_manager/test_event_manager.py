...
def task_function(callback, resource_id, **kwargs):
    ...
    if len(response) == 0:
        ...
        print(docs)
        docs = kwargs.get("faiss_service").do_search(
            query=f"{resource_id}", top_k=3, score_threshold=0.8
        )
        print(docs)
...
def test_event_manager():
    ...
    def register_and_execute_events(event_manager, start_idx, end_idx):
        ...
        for i in range(start_idx, end_idx):
            ...
            event_id = event_manager.register_event(
                ...
                kwargs={"faiss_service": faiss_service},
            )
    ...
    thread1 = threading.Thread(
        target=register_and_execute_events, args=(event_manager, 0, 5)
    )

    # Thread 2 - Registers and executes events from 5 to 9
    thread2 = threading.Thread(
        target=register_and_execute_events, args=(event_manager, 5, 10)
    )
